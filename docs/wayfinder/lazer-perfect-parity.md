...
- **T13 — Fix the 4 real lazer-parity bugs found by the audit (D1–D4)** (`task`, HITL) — Wire Score V2 production formula, remove HP loss cap, apply last-combo HP bonus, fix circle radius formula. `docs/wayfinder/tickets/T13-fix-audit-parity-bugs.md`
- **T14 — Decide the 5 webosu-extension-or-fix questions (D5–D9)** (`grilling`, HITL) — `sliderStyle` (keep as extension, document), `hitCircleOverlap` shift factor (fix to 1.0), `@2x` whitelist (extend), `[Colours] ApproachCircle` (drop mega task 5.12), `hit*-N.png` animated judgements (implement or document as skip). `docs/wayfinder/tickets/T14-decide-extension-questions.md`

## Open, blocked (not on the frontier)

- **T03 — Finish Track B skin conformance** (`task`, HITL) — blocked by T14. `docs/wayfinder/tickets/T03-skin-conformance-wiring.md`
- **T04 — Conformance harness: scene-graph snapshots, CI wiring, green on all reference skins** (`task`, HITL) — blocked by T03. `docs/wayfinder/tickets/T04-conformance-harness.md`
- **T06 — Rollout: flip the 4 feature flags on, remove legacy code, ship** (`grilling`, HITL) — blocked by T13, T03, T04. `docs/wayfinder/tickets/T06-rollout-flags.md`
- **T07 — Track C: latency probe + critical-path optimization** (`task`, HITL) — blocked by T05. `docs/wayfinder/tickets/T07-latency-probe-and-critical-path.md`
- **T08 — Research + decision: WebGPU renderer + OffscreenCanvas worker render in Pixi 8** (`research`, AFK) — blocked by T05, T07. `docs/wayfinder/tickets/T08-research-webgpu-offscreen.md`
- **T09 — AudioWorklet tight-clock: research + prototype decision** (`research`, AFK) — blocked by T05, T07. `docs/wayfinder/tickets/T09-research-audioworklet.md`
...
- **T11 — Real-play validation panel: 3 reference beatmaps × all flags on** (`grilling`, HITL) — blocked by T13, T04, T06. `docs/wayfinder/tickets/T11-real-play-validation.md`
