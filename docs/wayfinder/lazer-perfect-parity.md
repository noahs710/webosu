...
- **T15 — Implement D5/D6/D8 removals + fixes** (`task`, HITL) — delete the sliderStyle branch in `SliderMesh.js` + textured MeshRope code; fix hitCircleOverlap shift factor (`* 0.3` → `* 0.5`, default 0 → -2); remove beatmap ApproachCircle fallback from `playback.js`. Small focused changes; conformance goldens may shift. `docs/wayfinder/tickets/T15-implement-d5-d6-d8-removals.md`

## Open, blocked (not on the frontier)

- **T03 — Finish Track B skin conformance** (`task`, HITL) — blocked by T15 (D8 removal was originally in T03's scope; moving to T15 unblocks T03 to focus on D7 @2x extend). `docs/wayfinder/tickets/T03-skin-conformance-wiring.md`
...
- **T06 — Rollout: flip the 4 feature flags on, remove legacy code, ship** (`grilling`, HITL) — blocked by T15, T03, T04. `docs/wayfinder/tickets/T06-rollout-flags.md`
...
- **T16 — Implement D9 animated judgements** (`task`, HITL) — blocked by T03 (@2x whitelist should land first so animated frames' @2x variants load). `docs/wayfinder/tickets/T16-implement-d9-animated-judgements.md`
