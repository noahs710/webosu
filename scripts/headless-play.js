// Headless gameplay-launch verification: loads index-v2, fetches a real .osz from
// catboy.best (browser UA), calls window.launchGame, and watches for throws during
// extract -> parse -> audio decode -> Pixi app -> render loop. Captures pageerrors +
// screenshots the canvas. (FPS/feel still need real hardware; this catches gameplay-path
// crashes the build can't.)
const { spawn } = require("child_process");
const { chromium } = require("playwright");
const SET = 2006909, BID = 4174364, VER = "Lightspeed";
const vite = spawn(process.execPath, ["node_modules/vite/bin/vite.js","--port","5177","--strictPort"], { stdio:["ignore","pipe","pipe"] });
...
async function main(){
  if(!(await wait("http://localhost:5177/browse"))){console.log("vite not ready",viteErr);process.exit(1);}
  ...
  await p.goto("http://localhost:5177/browse",{waitUntil:"load",timeout:30000});
await p.waitForFunction(()=>typeof window.__ensureGame==="function", null, {timeout:15000}).catch(()=>{});
  ...
  console.log("bootstrap: launchGame="+typeof (await p.evaluate(()=>window.launchGame))+" skinReady="+await p.evaluate(()=>window.skinReady));
  ...
  // let it extract, parse, decode audio, create the pixi app, run the loop
  ...
  try { await p.screenshot({path:"scripts/headless-play.png"}); console.log("screenshot: scripts/headless-play.png"); } catch(e){}
