...
export default {
  ...
  setup() {
    ...
    function makeRandomSrc() {
      ...
      return `https://catboy.best/api/v2/search?q=&limit=8&offset=${offset}&status=1&status=3&status=4&mode=0`;
    }
    function randomize() {
    ...
    onMounted(async () => {
      if (window.localforage) {
        try {
          ...
          if (liked && liked.size) likedSids.value = Array.from(liked).slice(0, 8);
        } catch {}
        try {
          const hist = await new Promise(r => localforage.getItem("playhistory1000", (e, v) => r(v)));
          if (hist && hist.length) historySids.value = [...new Set(hist.map(h => h.sid).filter(Boolean))].slice(0, 8);
        } catch {}
      }
      randomSrc.value = makeRandomSrc();
    });
  ...
  template: `
    <div class="max-w-[1400px] mx-auto px-4 pt-2">
      ...
      <BeatmapList src="https://catboy.best/api/v2/search?q=&limit=8&offset=20&status=3&mode=0" :limit="8" />
      <h2 class="text-xl font-bold text-white mt-6 mb-2">New beatmaps</h2>
      <BeatmapList src="https://catboy.best/api/v2/search?q=&limit=8&status=4&mode=0" :limit="8" />
      <h2 class="text-xl font-bold text-white mt-6 mb-2">Recently played</h2>
      <BeatmapList :sids="historySids" :limit="8" empty-message="You haven't played any Beatmaps yet!" />
      <h2 class="text-xl font-bold text-white mt-6 mb-2">Favorites</h2>
      <BeatmapList :sids="likedSids" :limit="8" empty-message="You haven't favorited any Beatmaps yet!" />
      ...
      <BeatmapList :key="randomKey" :src="randomSrc" :limit="8" empty-message="No random maps found — hit Randomize to try again." />
