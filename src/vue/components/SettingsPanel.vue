...
const SLIDERS = [
   ["dim", "Background dim", 0, 100, 1, "%"],
   ["blur", "Background blur", 0, 100, 1, "%"],
   ["cursorsize", "Cursor size", 0.5, 2, 0.05, "x"],
   ["dpiscale", "Resolution", 0.5, 2, 0.05, "x"],
   ["mastervolume", "Master volume", 0, 100, 1, "%"],
   ["effectvolume", "Effect volume", 0, 100, 1, "%"],
   ["musicvolume", "Music volume", 0, 100, 1, "%"],
   ["audiooffset", "Audio offset", -200, 200, 1, "ms"],
];
...
const TOGGLE_LABELS = {
   ...
   spunout: "Spun Out",
   classic: "Classic",
   difficultyAdjust: "Difficulty Adjust",
   hideNumbers: "Hide numbers",
   hideGreat: "Hide 300s",
   hideFollowPoints: "Hide follow points",
   showTapIndicator: "Show tap indicator (hit circle + judgement)",
   snakein: "Snake-in",
   snakeout: "Snake-out",
   showhwmouse: "Hardware cursor",
   autofullscreen: "Auto fullscreen",
   sysdpi: "Use system resolution",
   beatmapHitsound: "Beatmap hitsounds",
};
...
const PATH_NAMESPACE = {
   dim: "display.dim",
   blur: "display.blur",
   cursorsize: "display.cursorsize",
   dpiscale: "display.dpiscale",
   mastervolume: "audio.mastervolume",
   effectvolume: "audio.effectvolume",
   musicvolume: "audio.musicvolume",
   audiooffset: "audio.audiooffset",
   hideNumbers: "display.hideNumbers",
   hideGreat: "display.hideGreat",
   hideFollowPoints: "display.hideFollowPoints",
   showTapIndicator: "display.showTapIndicator",
   snakein: "display.snakein",
   snakeout: "display.snakeout",
   showhwmouse: "display.showhwmouse",
   autofullscreen: "display.autofullscreen",
   sysdpi: "display.sysdpi",
   beatmapHitsound: "audio.beatmapHitsound",
   disableWheel: "input.disableWheel",
   disableButton: "input.disableButton",
   K1name: "input.K1name",
   K2name: "input.K2name",
   Kpausename: "input.Kpausename",
   Kpause2name: "input.Kpause2name",
   Kskipname: "input.Kskipname",
   K1keycode: "input.K1keycode",
   K2keycode: "input.K2keycode",
   Kpausekeycode: "input.Kpausekeycode",
   Kpause2keycode: "input.Kpause2keycode",
   Kskipkeycode: "input.Kskipkeycode",
   // mod flat flags
   autoplay: "mods.autoplay",
   easy: "mods.easy",
   hardrock: "mods.hardrock",
   nightcore: "mods.nightcore",
   daycore: "mods.daycore",
   hidden: "mods.hidden",
   nofail: "mods.nofail",
   suddendeath: "mods.suddendeath",
   perfect: "mods.perfect",
   spunout: "mods.spunout",
   classic: "mods.classic",
   difficultyAdjust: "mods.difficultyAdjust",
};
...
async function savePfp(url) {
   ...
   if (safe && !/^(https?:|\/)/i.test(safe)) {
      if (typeof window.__showErrorPopup === "function") {
         window.__showErrorPopup(
            "Only http(s) URLs or site-relative paths are allowed.",
            "Profile picture URL rejected",
         );
      }
      return;
   }
   ...
   try {
      ...
      setTimeout(() => {
         if (pfpUrlSaveStatus.value === "saved") pfpUrlSaveStatus.value = "";
      }, 1500);
   } catch (e) {
      pfpUrlSaveStatus.value = "error";
      if (typeof window.__showErrorPopup === "function") {
         window.__showErrorPopup(
            "Could not save profile picture: " + (e.message || e),
            "Profile picture",
         );
      } else {
      ...
      setTimeout(() => {
         if (pfpUrlSaveStatus.value === "error") pfpUrlSaveStatus.value = "";
      }, 3000);
   }
}
function captureKey(ev, key) {
   ...
   const handler = (e) => {
      ...
      if (keycodeKey !== key && keycodeKey in gamesettings)
         gamesettings[keycodeKey] = e.keyCode;
      GameState.set(PATH_NAMESPACE[key] || "settings." + key, name);
      if (keycodeKey !== key && keycodeKey in gamesettings) {
         GameState.set(
            PATH_NAMESPACE[keycodeKey] || "settings." + keycodeKey,
            e.keyCode,
         );
      }
      saveToLocal();
   ...
   const hasLocalChanges = (() => {
      try {
         ...
         return Object.keys(raw).some(
            (k) =>
               k in defaultsettings &&
               JSON.stringify(raw[k]) !== JSON.stringify(defaultsettings[k]),
         );
      } catch {
         return false;
      }
...
<template>
   <div class="space-y-4">
      <div class="bg-lazer-panel border border-white/5 rounded-xl p-4">
         ...
         <div
            v-for="s in SLIDERS.slice(0, 4)"
            :key="s[0]"
            class="flex items-center gap-2.5 my-2"
         >
         ...
         <div
            class="grid gap-1.5 mt-3"
            style="grid-template-columns: repeat(auto-fill, minmax(160px, 1fr))"
         >
            <label
               v-for="t in DISPLAY_TOGGLES"
               :key="t"
            ...
            >
               <input
                  type="checkbox"
                  :checked="gs[t]"
                  @change="set(t, $event.target.checked)"
               />
               {{ TOGGLE_LABELS[t] }}
            </label>
         </div>
      ...
      <div class="bg-lazer-panel border border-white/5 rounded-xl p-4">
         ...
         <label class="flex items-center gap-1.5 cursor-pointer mt-2">
            <input
               type="checkbox"
               :checked="gs.beatmapHitsound"
               @change="set('beatmapHitsound', $event.target.checked)"
            />
            Beatmap hitsounds
         </label>
      </div>
      ...
      <div class="bg-lazer-panel border border-white/5 rounded-xl p-4">
         ...
         <p class="text-lazer-dim text-sm">
            Manage skins on the
            <router-link to="/skins" class="text-lazer-pink hover:underline"
               >Skins page</router-link
            >. Default: reowoTuna. Mods are now in the sidebar (Mods button in
            the nav or F1).
         </p>
      </div>
      <div class="bg-lazer-panel border border-white/5 rounded-xl p-4">
         ...
         />
         <p class="text-lazer-dim text-xs">
            Paste an image link. Leave empty for initials avatar.
         </p>
      </div>
      <button
         @click="reset"
         class="bg-lazer-pink text-white rounded-lg px-4 py-2 text-sm hover:brightness-110"
      >
         Reset to defaults
      </button>
   </div>
