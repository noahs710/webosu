...
const TOGGLES = [
  ...
  ["showhwmouse", "Hardware cursor"], ["autofullscreen", "Auto fullscreen"],
];
...
class SettingsPanel extends LitElement {
  ...
  render() {
    return html`
      <div class="group"><h3>Display</h3>${SLIDERS.slice(0, 3).map(([k, label, min, max, step, unit]) => html`
        ...
        ${TOGGLES.filter(t => ["hideNumbers","hideGreat","hideFollowPoints","snakein","snakeout","showhwmouse","autofullscreen"].includes(t[0])).map(([k, label]) => html`
        <label class="tog"><input type="checkbox" ?checked=${gamesettings[k]} @change=${(e) => this._set(k, e.target.checked)} /> ${label}</label>`)}</div>
      <div class="group"><h3>Audio</h3>${SLIDERS.slice(3).map(([k, label, min, max, step, unit]) => html`
        <div class="row"><label>${label}</label><input type="range" min=${min} max=${max} step=${step} .value=${gamesettings[k]} @input=${(e) => this._set(k, +e.target.value)} /><span class="val">${gamesettings[k]}${unit}</span></div>`)}
