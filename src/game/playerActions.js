var checkClickdown = function checkClickdown() {
   ...
   try {
      useLegacy =
         new URLSearchParams(window.location.search).get("legacyinput") === "1";
   } catch {}
   var useRaw =
      useLegacy ||
      playback.autoplay ||
      playback.game.autopilot ||
      (playback.replayMode && playback.replayPlayback);
   var click;
   if (!useRaw && playback.game.mouse) {
      // predicted position (same as slider following uses)
      var t =
         playback && typeof playback.realtime === "number"
            ? playback.realtime
            : performance.now();
      var pred = playback.game.mouse(t);
      click = {
         x: pred.x,
         y: pred.y,
         time:
            playback.osu && playback.osu.audio
               ? playback.osu.audio.getPosition() * 1000
               : 0,
      };
   } else {
   ...
   if (playback.game.magnetised || playback.game.repel) {
      const mod = playback.game.magnetised
         ? window.ModRegistry
            ? window.ModRegistry.get("MG")
            : null
         : window.ModRegistry
           ? window.ModRegistry.get("RP")
           : null;
      const radius =
         (mod &&
            mod.settings &&
            (mod.settings.magnetRadius || mod.settings.repelRadius)) ||
         100;
      ...
      for (let i = 0; i < upcoming.length; i++) {
         ...
         const dx = h.x - click.x,
            dy = h.y - click.y;
         const d2 = dx * dx + dy * dy;
         if (d2 < radius2 && d2 < nearestD2) {
            nearest = h;
            nearestD2 = d2;
         }
      }
      if (nearest) {
         const dx = nearest.x - click.x,
            dy = nearest.y - click.y;
         const d = Math.sqrt(nearestD2);
         if (d > 0) {
            const factor = playback.game.magnetised
               ? Math.min(1, 0.5)
               : -Math.min(1, 0.5);
            click.x += dx * factor;
            click.y += dy * factor;
...
var inUpcoming = function (click) {
   return function (hit) {
      ...
      if (typeof click.x !== "number" || typeof click.y !== "number")
         return false;
      if (typeof click.time !== "number" || typeof hit.time !== "number")
         return false;
      if (
         typeof playback.circleRadius !== "number" ||
         typeof playback.MehTime !== "number"
      )
         return false;
      ...
      return (
         hit.score < 0 &&
         dx * dx + dy * dy < playback.hitRadius * playback.hitRadius &&
...
var inUpcoming_grace = function (predict) {
   return function (hit) {
      ...
      if (
         !predict ||
         typeof predict.x !== "number" ||
         typeof predict.y !== "number"
      )
         return false;
      if (typeof predict.time !== "number" || typeof hit.time !== "number")
         return false;
      if (
         typeof playback.circleRadius !== "number" ||
         typeof playback.MehTime !== "number"
...
var playerActions = function (playback) {
   ...
   playback.game.updatePlayerActions = function (time) {
      ...
      if (playback.game.relax) {
         ...
         for (var i = 0; i < upcoming.length; i++) {
            ...
            var inRange =
               dx * dx + dy * dy <
               playback.hitRadius * playback.hitRadius;
            var clickTime =
               playback.osu && playback.osu.audio
                  ? playback.osu.audio.getPosition() * 1000
                  : 0;
   ...
   playback.game.mouseInto = function (t, out) {
      ...
      if (i == 0) {
         ...
         vx = (m[0].x - m[i].x) / (m[0].t - m[i].t);
         vy = (m[0].y - m[i].y) / (m[0].t - m[i].t);
      }
      let dt = Math.min(t - m[0].t + window.currentFrameInterval, 40);
      out.x = m[0].x + vx * dt;
      out.y = m[0].y + vy * dt;
      out.r =
         Math.hypot(vx, vy) * Math.max(t - m[0].t, window.currentFrameInterval);
      return out;
   };
   ...
   playback.game.cleanupPlayerActions = function () {
      ...
      playback.game.window.removeEventListener("touchcancel", touchendCallback);
   };
};
