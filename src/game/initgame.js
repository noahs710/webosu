...
try {
   ...
   ilog("initgame", "applied gamesettings to game", {
      autoplay: game.autoplay,
      hidden: game.hidden,
   });
} catch (e) {
   iwarn("initgame", "gamesettings loadToGame failed", e);
}
...
async function loadDefaultSkin() {
   ...
   try {
      ...
      // A stale or empty cache (e.g. from an older session) returns a non-null record with no textures.
      // Treat it as a miss and re-fetch the OSK so the validator never sees 0 textures.
      const cachedTexCount = cached
         ? Object.keys(cached.textures || {}).length
         : 0;
      if (cached && cachedTexCount > 0) {
         ilog(
            "initgame",
            "cached skin found",
            cached.config?.name || "unnamed",
            "textures",
            cachedTexCount,
         );
         ...
         return true;
      } else if (cached && cachedTexCount === 0) {
         iwarn(
            "initgame",
            "cached skin is empty (0 textures), falling back to fetch",
         );
      }
   } catch (e) {
   ...
   try {
      ...
      ilog(
         "initgame",
         "default .osk loaded",
         data.config?.name || "unnamed",
         "textures",
         Object.keys(data.textures || {}).length,
      );
      try {
         await saveLocalSkin(data, "reowoTuna.osk");
      } catch (e) {
      ...
      iwarn(
         "initgame",
         "default .osk load failed, falling back to sprites.json",
         e,
      );
   }
    ...
    try {
      ...
      ilog(
         ...
         Object.keys(window.Skin).length,
         "textures",
      );
      return true;
   } catch (e) {
      ierror("initgame", "sprites.json also failed — game cannot start", e);
      return false;
   }
}
loadDefaultSkin().then((ok) => {
   ...
   try {
      ...
      if (!result.ok) {
         ilog(
            "initgame",
            "skin validation issue",
            result,
            "window.Skin keys",
            Object.keys(window.Skin || {}).slice(0, 20),
         );
         window.dispatchEvent(
            new CustomEvent("skin-health-issue", {
               detail: {
                  type: "skin",
                  missing: result.missing,
                  corrupt: result.corrupt,
                  message: result.missing.length
                     ? "Missing core textures: " + result.missing.join(", ")
      ...
      iwarn("initgame", "skin validation failed", e);
   }
});
         ...
         window.dispatchEvent(
            new CustomEvent("skin-health-issue", {
               detail: {
                  ...
                  missing: result.missing,
                  message:
                     "Missing core hitsounds: " + result.missing.join(", "),
               },
            }),
         );
      }
   } catch (e) {
      iwarn("initgame", "hitsound validation failed", e);
   }
};
sounds.load(sample);
...
function resumeHitsoundContext() {
   ...
   try {
      ...
      /* ignore — autoplay policy race; the next gesture will retry */
   }
}
...
window.addEventListener("touchend", resumeHitsoundContext, {
   once: true,
   passive: true,
});
