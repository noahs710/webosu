...
import CircumscribedCircle from "./curves/CircumscribedCircle.js";
   ...
   function Track(zip, track) {
      ...
      this.decode = (function decode() {
         // Decodes a .osu file
         var lines = self.track.replace(/\r/g, "").split("\n");
            ...
            switch (section) {
               ...
               case "[HitObjects]":
                  ...
                  if ((hit.type & HIT_TYPE_CIRCLE) > 0) {
                     ...
                     // parse hitSample
                     const hitSample = (
                        parts.length > 5 ? parts[5] : "0:0:0:0:"
                     ).split(":");
                     hit.hitSample = {
                     ...
                     // parse hitSample
                     const hitSample = (
                        parts.length > 10 ? parts[10] : "0:0:0:0:"
                     ).split(":");
                     hit.hitSample = {
                        normalSet: +hitSample[0],
                        additionSet: +hitSample[1],
                     ...
                     // parse hitSample
                     const hitSample = (
                        parts.length > 6 ? parts[6] : "0:0:0:0:"
                     ).split(":");
                     hit.hitSample = {
                        normalSet: +hitSample[0],
                        additionSet: +hitSample[1],
         ...
         if (this.difficulty.OverallDifficulty) {
            this.difficulty.HPDrainRate =
               this.difficulty.HPDrainRate || this.difficulty.OverallDifficulty;
            this.difficulty.CircleSize =
               this.difficulty.CircleSize || this.difficulty.OverallDifficulty;
         ...
         for (var i = 0; i < this.timingPoints.length; i++) {
            ...
            if (point.uninherited === 0) {
               point.uninherited = 1;
               point.millisecondsPerBeat = Math.min(
                  point.millisecondsPerBeat,
                  -10
               );
            ...
            if (hit.type == "slider") {
               hit.sliderTime =
                  (hit.timing.millisecondsPerBeat *
                     (hit.pixelLength / this.difficulty.SliderMultiplier)) /
                  100;
   ...
   function preallocateTiming(track) {
      ...
      for (let i = 0; i < track.hitObjects.length; ++i) {
         while (
            ...
            track.timingPoints[currentTimingIndex + 1].offset <=
               track.hitObjects[i].time
         ) {
            ...
            if (hit.sliderType === "P" && hit.keyframes.length == 2) {
               ...
               if (hit.sliderType == "C")
                  if (import.meta.env.DEV) console.warn(
                     ...
                     "Catmull curve unsupported. fallback to bezier"
                  );
               hit.curve = new LinearBezier(hit, hit.sliderType === "L");
            }
   ...
   function stackHitObjects(track) {
      ...
      for (let i = 0; i < track.hitObjects.length; ++i) {
         ...
         for (let j = i + 1; j < track.hitObjects.length; ++j) {
            ...
            if (getintv(newchain[newchain.length - 1], hitJ) > stackThreshold)
               break;
         ...
         if (newchain.length > 1) {
            // just ignoring one-element chains
      ...
      for (let i = 0; i < chains.length; ++i) {
         if (chains[i][0].type == "slider") {
            ...
            for (let j = 0, dep = 0; j < chains[i].length; ++j) {
               ...
               if (
                  chains[i][j].type != "slider" ||
                  chains[i][j].repeat % 2 == 0
               )
                  dep++;
            }
         } else {
            ...
            for (let j = 0, dep = 0; j < chains[i].length; ++j) {
               ...
               if (
                  j > 0 &&
                  chains[i][cur].type == "slider" &&
                  chains[i][cur].repeat % 2 == 1
               )
                  dep--;
