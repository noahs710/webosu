...
#### Scenario: Reject zip bomb
- **WHEN** a `.osk` contains 500 files or unzipped >50MB
- **THEN** the system rejects with "too many files" or "unzipped too large"
...
The system SHALL create blob URLs from extracted PNG file buffers and load them as PIXI textures via `PIXI.Assets.load` (not `PIXI.Texture.from` with `Image` race). Textures SHALL override the corresponding entries in `window.Skin` after `await` and `revokeObjectURL` after upload. Only gameplay-relevant textures (hit, default-, score, cursor, slider, followpoint 0-9, etc.) SHALL be loaded, capped at 120, skipping `hit*-*.png` numbered variants and `followpoint>9`.

#### Scenario: Blob URL texture overrides Skin
- **WHEN** a mapped PNG is extracted
- **THEN** `window.Skin[mappedKey]` becomes the new `PIXI.Texture` after `Assets.load` and old texture `destroy(false)` is called
